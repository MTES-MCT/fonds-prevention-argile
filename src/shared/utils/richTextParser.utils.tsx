...
const richTextParser: RichTextParserFunction = (keyTranslation) => {
  ...
  keyTranslation.replace(regex, (match, boldText, aHref, aAttributes, aText, offset) => {
    if (lastIndex < offset) {
    ...
    if (aHref && aText) {
      ...
      const rel = relMatch ? relMatch[1] : target === "_blank" ? "noopener noreferrer" : undefined;

      parts.push(
        <a className="[&::after]:hidden!" href={aHref} key={offset} rel={rel} target={target}>
          {aText}
        </a>
      );
    }
