...
export const amoValidationTokens = pgTable(
  ...
  (table) => ({
    ...
    validationIdx: index("amo_validation_validation_id_idx").on(table.parcoursAmoValidationId),
  })
);

// Relations : un token appartient à une validation AMO
export const amoValidationTokensRelations = relations(amoValidationTokens, ({ one }) => ({
