...
export class UserRepository extends BaseRepository<User> {
  ...
  async claimStub(userId: string, userInfo: FranceConnectUserInfo): Promise<User> {
    const result = await db
      ...
      .set({
        ...
        nom: userInfo.usual_name || userInfo.family_name || sql`${users.nom}`,
   ...
   * @param options.claimToken - Jeton de rattachement d'un dossier pré-créé par un AV.
   */
  async upsertFromFranceConnect(
    ...
    if (existingUser) {
      const updates: Partial<NewUser> = {
        ...
        nom: userInfo.usual_name || userInfo.family_name || existingUser.nom,
    ...
    return await this.create({
      ...
      nom: userInfo.usual_name || userInfo.family_name,
