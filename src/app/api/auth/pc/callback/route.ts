...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(new URL(`${ROUTES.connexion.agent}?error=${errorCode}`, request.url));
  }

  // Vérification des paramètres requis
  if (!code || !state) {
    return NextResponse.redirect(new URL(`${ROUTES.connexion.agent}?error=pc_missing_params`, request.url));
  }
  ...
  try {
    ...
    if (!result.success) {
      ...
      if (result.shouldLogout) {
        return NextResponse.redirect(new URL(`${ROUTES.connexion.agent}?error=pc_security_error`, request.url));
      }

      return NextResponse.redirect(new URL(`${ROUTES.connexion.agent}?error=${result.error}`, request.url));
    }
    ...
    return NextResponse.redirect(new URL(`${ROUTES.connexion.agent}?error=pc_auth_failed`, request.url));
  }
}
