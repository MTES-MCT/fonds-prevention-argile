...
interface AmoAccueilHeaderProps {
  nombreDemandesEnAttente: number;
  nombreDossiersSuivis: number;
}

export function AmoAccueilHeader({ nombreDemandesEnAttente, nombreDossiersSuivis }: AmoAccueilHeaderProps) {
  const { user } = useAuth();
    ...
    <div className="fr-container fr-py-4w">
      ...
      <div className="fr-grid-row fr-grid-row--gutters fr-mt-4w">
        <div className="fr-col-12 fr-col-md-6">
          <StatTile
            number={nombreDemandesEnAttente.toLocaleString("fr-FR")}
            label="Demandes d'accompagnement"
            description="🔥 À traiter en priorité"
          />
        </div>
        <div className="fr-col-12 fr-col-md-6">
          <StatTile number={nombreDossiersSuivis.toLocaleString("fr-FR")} label="Dossiers suivis au total" />
        </div>
      </div>
    </div>
  );
