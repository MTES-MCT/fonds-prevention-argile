...
interface AllersVersEditModalProps {
  allersVers: AllersVersWithRelations;
  onClose: () => void;
  onSuccess: () => void;
}

export function AllersVersEditModal({ allersVers, onClose, onSuccess }: AllersVersEditModalProps) {
  const modalId = `modal-edit-allers-vers`;
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const [formData, setFormData] = useState({
    nom: allersVers.nom,
    departements: allersVers.departements?.map((d) => d.codeDepartement).join(", ") || "",
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="fr-input-group">
                    <label className="fr-label" htmlFor={`${modalId}-departements`}>
                      <strong>Codes Départements</strong> *
                    </label>
                    <input
                      className="fr-input"
                      type="text"
