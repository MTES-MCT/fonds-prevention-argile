...
} from "../domain/matomo-funnels.types";
...
export async function getFunnelSimulateurRGA(): Promise<FunnelStatistiques> {
  try {
    ...
    const FUNNEL_ID = serverEnv.MATOMO_MES_AIDES_RENO_FUNNEL_ID;
    ...
    return transformMatomoFunnelData(
      funnelData as MatomoFunnelFlowTableResponse
    );
  } catch (error) {
 ...
 */
function transformMatomoFunnelData(
  data: MatomoFunnelFlowTableResponse
): FunnelStatistiques {
  ...
  const etapes: FunnelStep[] = data.map((step) => {
    ...
    const abandons =
      typeof step.step_nb_exits === "string"
        ? parseInt(step.step_nb_exits, 10)
        : step.step_nb_exits;

    // Parser les taux (format: "70 %")
    const tauxConversion = step.step_proceeded_rate
      ? parseFloat(
          step.step_proceeded_rate
            .replace(/\s/g, "")
            .replace("%", "")
            .replace(",", ".")
        )
  ...
  const tauxConversionGlobal =
    visiteursInitiaux > 0
      ? Math.round((conversionsFinales / visiteursInitiaux) * 100 * 100) / 100
      : 0;
