...
import type { MatomoFunnelFlowTableResponse } from "../domain/matomo-funnels.types";
...
export async function fetchMatomoFunnel(
  ...
  const MATOMO_MES_AIDES_RENO_API_TOKEN =
    serverEnv.MATOMO_MES_AIDES_RENO_API_TOKEN;
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Erreur API Matomo Funnel: ${response.status} ${response.statusText}`
    );
  }
