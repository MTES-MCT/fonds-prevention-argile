...
export async function getUsersWithParcours(scopeFilters?: ScopeFilters | null): Promise<UserWithParcoursDetails[]> {
  ...
  let query = db
    .select({
      ...
      userPartnerSource: users.partnerSource,
      ...
      parcoursSituationParticulier: parcoursPrevention.situationParticulier,
      ...
      dossierDsDemarcheId: dossiersDemarchesSimplifiees.dsDemarcheId,
      dossierDsStatus: dossiersDemarchesSimplifiees.dsStatus,
      dossierSubmittedAt: dossiersDemarchesSimplifiees.submittedAt,
      dossierInstructedAt: dossiersDemarchesSimplifiees.instructedAt,
  ...
  for (const row of scopedResults) {
    ...
    if (!usersMap.has(row.userId)) {
      usersMap.set(row.userId, {
        user: {
          ...
          sourceAcquisition: row.userSourceAcquisition,
          sourceAcquisitionPrecision: row.userSourceAcquisitionPrecision,
          partnerSource: row.userPartnerSource,
        ...
        parcours: row.parcoursId
          ? {
              ...
              situationParticulier: row.parcoursSituationParticulier!,
    ...
    if (row.dossierId && row.dossierStep) {
      const dossierInfo: DossierInfo = {
        ...
        dsDemarcheId: row.dossierDsDemarcheId!,
        dsStatus: row.dossierDsStatus!,
        submittedAt: row.dossierSubmittedAt,
        instructedAt: row.dossierInstructedAt,
