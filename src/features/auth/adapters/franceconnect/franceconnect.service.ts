...
import { emitBrevoEvent, BREVO_EVENTS, BREVO_ATTRS, buildConseillerEventProperties } from "@/shared/email/brevo";
...
export async function handleFranceConnectCallback(
  ...
  try {
    ...
    if (created) {
      ...
      const conseillerEventProperties = await buildConseillerEventProperties(parcours.id);
      await emitBrevoEvent(parcours.id, BREVO_EVENTS.DEMANDEUR_CREE, {
        attributes: { [BREVO_ATTRS.A_AMO]: false },
        ...(Object.keys(conseillerEventProperties).length > 0
          ? { eventProperties: conseillerEventProperties }
          : {}),
      });
    }
