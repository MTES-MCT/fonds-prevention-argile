...
describe("etat-maison.rules", () => {
  describe("checkEtatMaison", () => {
    ...
    it("retourne passed pour une maison endommagée", () => {
      const result = checkEtatMaison("endommagée");
      expect(result.passed).toBe(true);
      expect(result.reason).toBeUndefined();
    });

    it("retourne failed pour une maison très endommagée", () => {
      const result = checkEtatMaison("très endommagée");
      expect(result.passed).toBe(false);
      expect(result.reason).toBe(EligibilityReason.MAISON_TROP_ENDOMMAGEE);
    });
