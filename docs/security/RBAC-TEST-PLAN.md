...
| Départements non couverts (ADR-0034)                   | `administration/demandeurs/actions/couverture-territoriale.actions.ts`                                   | aggregate   | sans `USERS_READ` → DENY, sans requête base                                                                                                                                                                            | OUI (testé)                                                                               |
| Simulation du demandeur (lecture + écriture, ADR-0036) | `parcours/core/services/ma-simulation.service.ts`, `actions/enregistrer-simulation-demandeur.actions.ts` | individual  | non-auth → DENY ; parcours résolu par la **session**, aucun id accepté du client (pas d'IDOR) ; correction agent **complète**, décision AMO rendue ou formulaire DN commencé (jusqu'à la décision DDT) → écriture DENY | OUI (`ma-simulation.service.test.ts`, `enregistrer-simulation-demandeur.actions.test.ts`) |
| Inéligibilité affichée au demandeur (ADR-0034/0036)    | `parcours/core/actions/eligibilite-query.actions.ts`                                                     | individual  | non-auth → DENY ; parcours résolu par la session ; ne lit que l'archivage du parcours, jamais la validation AMO (une décision positive ne doit pas ressortir en inéligibilité)                                         | OUI (`eligibilite-query.actions.test.ts`)                                                 |
