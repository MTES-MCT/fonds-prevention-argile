...
| Simulation du demandeur (lecture + écriture, ADR-0036) | `parcours/core/services/ma-simulation.service.ts`, `actions/enregistrer-simulation-demandeur.actions.ts` | individual  | non-auth → DENY ; parcours résolu par la **session**, aucun id accepté du client (pas d'IDOR) ; correction agent **complète**, décision AMO rendue ou formulaire chez la DDT → écriture DENY | OUI (`ma-simulation.service.test.ts`, `enregistrer-simulation-demandeur.actions.test.ts`) |
