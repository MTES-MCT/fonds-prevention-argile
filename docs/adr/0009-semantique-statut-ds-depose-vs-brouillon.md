...
4. **La sync DS écrit les dates** : `getDossierStatus()` remonte `{ state, datePassageEnConstruction, datePassageEnInstruction }` ; `updateDossierStatus` écrit `submitted_at` / `instructed_at` via le **mapper de colonne Drizzle** (`Date` typée, comme `last_sync_at`), même lorsque le statut est inchangé — réécriture idempotente, ces dates étant immuables côté DS. Ne **jamais** interpoler un `Date` dans un `sql` brut (`COALESCE(...)`) : postgres.js ne sait pas le sérialiser et fait planter tout l'UPDATE (`ERR_INVALID_ARG_TYPE`), si bien que `ds_status` **et** les dates ne sont jamais persistés. `recomputeParcoursStatus` ignore les dossiers sans statut.
