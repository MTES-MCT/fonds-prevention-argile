...
- **`period=range` n'est jamais pré-archivé par Matomo** (contrairement à `day`/`week`/`month`, pré-calculés par le cron d'archivage) : toujours recalculé en live, pire cas combiné à un `segment` (département) sur une longue période — c'est la cause principale des timeouts du dashboard admin au changement de filtre département, pas seulement un problème de granularité du graphique (ADR-0030). Tout comptage **additif** (events, `nb_visits`) doit utiliser `getGranulariteForPeriode` + sommer les sous-périodes plutôt que `period=range` — cf. `fetchMatomoEvents`/`fetchMatomoEventsByDepartment` (`matomo-api.adapter.ts`, fonction `sumEventCounts`). Un dédoublonnage (visiteurs uniques, `VisitsSummary.get`) n'est **pas** additif : sommer des sous-périodes surestimerait un visiteur revenant sur plusieurs jours — reste en `period=range`, la vraie option pour le rendre rapide est de pré-archiver les segments département côté Matomo (admin Matomo, hors code applicatif, cf. ADR-0030)
- Un timeout Matomo (`period=range`) n'interrompt pas l'archivage côté Matomo : il continue en tâche de fond, et un nouvel essai peu après réussit souvent (l'archive est alors prête) — c'est le contournement utilisé en pratique. En conséquence, **ne jamais mettre en cache un échec Matomo** (`fetchMatomoApiCached`, `matomo-api.adapter.ts`) : ça bloquerait ce contournement jusqu'à expiration du cache. Envisagé puis explicitement écarté, cf. ADR-0030 (Option D bis)
- **Sur une réponse Matomo multi-sous-période** (`period=day`/`week`/`month` avec un `date` en plage — donc un objet keyed par sous-période, pas un tableau plat), les métriques (`nb_visits`, visites) peuvent être sérialisées en **string** plutôt qu'en nombre, contrairement au format `period=range`/single-period où ce sont déjà des nombres. Toujours `Number(...)` avant de sommer/additionner — sans ça, `(total ?? 0) + valeur` fait de la **concaténation de texte** silencieuse (`0 + "234"` → `"0234"`) au lieu d'une addition, repéré en préprod sur `sumEventCounts` (`matomo-api.adapter.ts`) et `getMatomoStatistiques` (`matomo.service.ts`)
