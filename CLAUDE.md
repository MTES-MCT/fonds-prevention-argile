...
- `@gouvfr/dsfr-chart` (`<line-chart>`) n'expose **aucun style de segment/ligne** (pas de `borderDash`, pas de prop de style par point) — vérifié dans le bundle (pas de `.d.ts`, pas de doc dessus) : chaque dataset est construit en dur (`borderWidth: 2`, pas de dash). Pour distinguer visuellement un tronçon (ex. mois en cours incomplet sur `/stats`), seule option côté composant : une **2e série qui chevauche le point de jonction**, avec une couleur différente via `selectedPalette="categorical"` (`dsfr-chart-colors-01`/`-02`, cf. `build-chart-series.utils.ts`) — un vrai pointillé demanderait de sortir du composant et d'utiliser Chart.js directement (déjà présent en interne, v4.5.1, option native `segment.borderDash`)
- **maplibre 6 n'échoue plus bruyamment sans WebGL2**, et c'est un piège : là où la v5 levait depuis `new Map()`, la v6 construit une carte **sans `painter`** et se contente de `fire` un `GPUInitializationError` — donc un simple `console.error` si personne n'écoute l'évènement. La panne n'éclate qu'ensuite, dans son propre `remove()`, qui déréférence ce `painter` absent : partant d'un **cleanup d'effet React**, l'erreur remonte jusqu'à la frontière racine et `global-error.tsx` remplace **toute la page** par « Application error: a client-side exception has occurred ». Constaté en production sur des demandeurs Firefox (GPU sur liste noire / accélération coupée) alors qu'Edge passait sur la même machine, et déclenché par un geste anodin : re-modifier l'adresse démonte la carte. `useRgaMap` teste donc WebGL2 **avant** d'instancier (`webgl2EstDisponible`, `rga-map/domain/webgl.ts`) et enveloppe `remove()` — les deux, pas l'un ou l'autre : le contexte peut aussi se perdre en cours de route. Sans WebGL2, `RgaMap` rend un message à la place du canvas et `StepAdresse` propose immédiatement la saisie manuelle (repli déjà existant, qui ne dépend que des coordonnées BAN)
- `cookies()` **signale l'usage dynamique en lançant** une erreur de digest `DYNAMIC_SERVER_USAGE` : c'est ainsi que Next bascule une route de statique à dynamique au build. Un `try/catch` générique autour d'une lecture de session **avale ce signal** — la page se prérend alors avec « pas d'agent » et se fige en **redirection statique**, build vert à l'appui. Symptôme visible avant la panne : `[getCurrentAgent] Erreur: … couldn't be rendered statically`. Tout catch traversé pendant un rendu RSC appelle donc `unstable_rethrow(error)` (`next/navigation`) en tête (`agent.actions.ts`, `dossier-detail.service.ts`, `prospect-detail.service.ts`, `edition-simulation.service.ts`) — et toute route derrière une session déclare `export const dynamic = "force-dynamic"` plutôt que de compter sur ce throw
- **`next/navigation` n'a pas le même `unstable_rethrow` côté serveur et côté navigateur** : la variante navigateur ignore `DYNAMIC_SERVER_USAGE` (sans objet hors prerender). Sous vitest (`environment: "jsdom"`) c'est **elle** qui est résolue, y compris via `importOriginal` — un test qui s'appuierait dessus validerait l'inverse de la prod. Le mock global de `vitest.setup.ts` reproduit donc le tri **serveur**
